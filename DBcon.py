...
class DataConnect:
    ...
    def csen(self, ky, kr):
        self.cur.execute("INSERT INTO poom (uou, ku) VALUES (%(uou)s, %(ku)s, NOW(), %(poom)s);", {'uou': ky, 'ku': kr, 'poom': 2})
        self.conn.commit()
    ...
    def sai_word_type(self, word, cha):
        wtp = [x for x, y in self.word_type()]
        if word not in wtp:
            self.cur.execute("INSERT INTO word_data (word ,word_type) VALUES (%(word)s, "
                             "%(cha)s);", {'word': word, 'cha': cha})
            self.conn.commit()
            print("เพิ่มคำว่า " + word + " ลงใน word_data แล้ว")
        else:
            print("มีคำว่า " + word + " อยู่ใน word_data แล้ว")
